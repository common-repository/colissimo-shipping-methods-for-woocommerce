...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	public static function _parsejpeg($file) {
		...
		while (($pos = strpos($data, "ICC_PROFILE\0", $offset)) !== false) {
			...
			$length = (LPC_TCPDF_STATIC::_getUSHORT($data, ($pos - 2)) - 16);
	...
	public static function _parsepng($file) {
		...
		$w = LPC_TCPDF_STATIC::_freadint($f);
		$h = LPC_TCPDF_STATIC::_freadint($f);
		$bpc = ord(fread($f, 1));
		$ct = ord(fread($f, 1));
		if ($ct == 0) {
			$colspace = 'DeviceGray';
		...
		$data = '';
		$icc = false;
		$n = LPC_TCPDF_STATIC::_freadint($f);
		do {
			$type = fread($f, 4);
			if ($type == 'PLTE') {
				// read palette
				$pal = LPC_TCPDF_STATIC::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'tRNS') {
				// read transparency info
				$t = LPC_TCPDF_STATIC::rfread($f, $n);
				if ($ct == 0) { // DeviceGray
					$trns = array(ord($t[1]));
				...
				$data .= LPC_TCPDF_STATIC::rfread($f, $n);
				fread($f, 4);
			} elseif ($type == 'iCCP') {
				// skip profile name
				$len = 0;
				...
				$icc = LPC_TCPDF_STATIC::rfread($f, ($n - $len - 2));
				// decompress profile
