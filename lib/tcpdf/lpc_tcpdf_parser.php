...
require_once(dirname(__FILE__).'/include/lpc_tcpdf_filters.php');

/**
 * @class LPC_TCPDF_PARSER
 * This is a PHP class for parsing PDF documents.<br>
 * @package com.tecnick.tcpdf
...
// -----------------------------------------------------------------------------
	...
	protected function decodeStream($sdic, $stream) {
		...
		foreach ($filters as $filter) {
			if (in_array($filter, LPC_TCPDF_FILTERS::getAvailableFilters())) {
				try {
					$stream = LPC_TCPDF_FILTERS::decodeFilter($filter, $stream);
				} catch (Exception $e) {
					$emsg = $e->getMessage();
	...
	public function Error($msg) {
		if ($this->cfg['die_for_errors']) {
			die('<strong>LPC_TCPDF_PARSER ERROR: </strong>'.$msg);
		} else {
			throw new Exception('LPC_TCPDF_PARSER ERROR: '.$msg);
		}
	}

} // END OF LPC_TCPDF_PARSER CLASS
