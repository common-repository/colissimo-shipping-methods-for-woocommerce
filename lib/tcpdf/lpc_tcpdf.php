...
class LPC_TCPDF {
	...
	public function setCellPaddings($left=null, $top=null, $right=null, $bottom=null) {
		if (!LPC_TCPDF_STATIC::empty_string($left) AND ($left >= 0)) {
			$this->cell_padding['L'] = $left;
		}
		if (!LPC_TCPDF_STATIC::empty_string($top) AND ($top >= 0)) {
			$this->cell_padding['T'] = $top;
		}
		if (!LPC_TCPDF_STATIC::empty_string($right) AND ($right >= 0)) {
			$this->cell_padding['R'] = $right;
		}
		if (!LPC_TCPDF_STATIC::empty_string($bottom) AND ($bottom >= 0)) {
	...
	/**
	 * Throw an exception or print an error message and die if the K_LPC_TCPDF_PARSER_THROW_EXCEPTION_ERROR constant is set to true.
	...
	public function Error($msg) {
		...
		$this->_destroy(true);
		if (defined('K_LPC_TCPDF_THROW_EXCEPTION_ERROR') AND !K_LPC_TCPDF_THROW_EXCEPTION_ERROR) {
	...
	public function Ln($h=null, $cell=false) {
		...
		if (LPC_TCPDF_STATIC::empty_string($h)) {
			$h = $this->lasth;
		}
		$this->y += $h;
		$this->newline = true;
		...
		if ($destroyall AND !$preserve_objcopy && isset($this->file_id)) {
			...
			if (isset($this->imagekeys)) {
				foreach($this->imagekeys as $file) {
					if (strpos($file, K_PATH_CACHE) === 0 && LPC_TCPDF_STATIC::file_exists($file)) {
						@unlink($file);
					}
				}
			}
	...
	protected function _putpages() {
		...
		for ($n = 1; $n <= $num_pages; ++$n) {
			...
			if (!empty($this->pagegroups)) {
				...
				list($temppage, $gdiff) = LPC_TCPDF_STATIC::replacePageNumAliases($temppage, $replace, $gdiff);
			}
			// replace page numbers
			$replace = array();
			...
			list($temppage, $pdiff) = LPC_TCPDF_STATIC::replacePageNumAliases($temppage, $replace, $pdiff);
	...
	protected function _getobj($objid=null) {
		if (LPC_TCPDF_STATIC::empty_string($objid)) {
			++$this->n;
			$objid = $this->n;
		}
	...
	public function write1DBarcode($code, $type, $x=null, $y=null, $w=null, $h=null, $xres=null, $style=array(), $align='') {
		...
		require_once(dirname(__FILE__).'/lpc_tcpdf_barcodes_1d.php');
		// save current graphic settings
		$gvars = $this->getGraphicVars();
		// create new barcode object
		$barcodeobj = new LPC_TCPDFBarcode($code, $type);
		...
		// height
		if (LPC_TCPDF_STATIC::empty_string($h) OR ($h <= 0)) {
			// set default height
	...
	public function write2DBarcode($code, $type, $x=null, $y=null, $w=null, $h=null, $style=array(), $align='', $distort=false) {
		...
		require_once(dirname(__FILE__).'/lpc_tcpdf_barcodes_2d.php');
		// save current graphic settings
		$gvars = $this->getGraphicVars();
		// create new barcode object
		$barcodeobj = new LPC_TCPDF2DBarcode($code, $type);
		$arrcode = $barcodeobj->getBarcodeArray();
		...
		// set dimensions
		if ((LPC_TCPDF_STATIC::empty_string($w) OR ($w <= 0)) AND (LPC_TCPDF_STATIC::empty_string($h) OR ($h <= 0))) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if (!empty($css)) {
						...
						list($dom[$key]['csssel'], $dom[$key]['cssdata']) = LPC_TCPDF_STATIC::getCSSdataArray($dom, $key, $css);
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	protected function openHTMLTagHandler($dom, $key, $cell) {
		...
		switch($tag['value']) {
			...
			case 'tcpdf': {
				if (defined('K_LPC_TCPDF_CALLS_IN_HTML') AND (K_LPC_TCPDF_CALLS_IN_HTML === true)) {
	...
	public function selectColumn($col=null) {
		if (LPC_TCPDF_STATIC::empty_string($col)) {
	...
	*/
    ...
    protected function getCachedFileContents($file)
    {
        if (!isset($this->fileContentCache[$file])) {
            $this->fileContentCache[$file] = LPC_TCPDF_STATIC::fileGetContents($file);
        }
    ...
    protected function fileExists($file)
    {
        ...
        return LPC_TCPDF_STATIC::file_exists($file);
    }

} // END OF LPC_TCPDF CLASS
